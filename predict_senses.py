...
class DiscourseSenseModel:
    def __init__(self, tokenizer, sense_model_embed, sense_model, device='cpu'):
    ...
    def load_model(save_path, relation_type='altlex', device='cpu'):
        ...
        return DiscourseSenseModel(tokenizer, sense_model_embed, sense_model, device)
    ...
    def predict(self, doc):
        ...
        for par_i, paragraph in enumerate(doc):
...
def main(corpus, save_path, replace, output_path, limit):
    ...
    signal_model = DiscourseSenseModel.load_model(save_path, device=device)
